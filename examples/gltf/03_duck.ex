defmodule EAGL.Examples.GLTF.Duck do
  ...
  Builds on Example 2 by testing:
  - Multi-node scene graph with parent-child transforms
  - Node transform hierarchy (TRS properties)
  - Larger mesh with more complex geometry
  - Scene.render handling the full transform hierarchy

  Run with: mix run examples/gltf/03_duck.ex
  """
  ...
  def run_example(opts \\ []) do
    ...
    EAGL.Window.run(__MODULE__, "EAGL GLTF Example 3: Duck", merged_opts)
  end
  ...
  defp create_shader_program do
    ...
    with {:ok, vs} <- create_shader_from_source(@gl_vertex_shader, vs_source, "duck_vs"),
         {:ok, fs} <- create_shader_from_source(@gl_fragment_shader, fs_source, "duck_fs"),
  ...
  defp load_gltf_scene(program) do
    with {:ok, gltf, _glb, data_store} <- load_glb_file(),
         {:ok, {scene, all_nodes}} <- GLTF.EAGL.to_scene(gltf, data_store),
         {:ok, textures} <- load_textures(gltf, data_store) do
      updated_roots =
      ...
      {:ok, scene, textures, "#{node_count} nodes, #{mesh_count} mesh(es), #{tex_count} texture(s)"}
    end
  end
  ...
  defp load_textures(gltf, data_store) do
    textures = %{}

    textures =
      case load_first_base_color_texture(gltf, data_store) do
  ...
  defp load_first_base_color_texture(gltf, data_store) do
    with material when not is_nil(material) <- Enum.at(gltf.materials || [], 0),
      ...
      case load_texture_from_binary(image_binary, mime_type: image.mime_type) do
