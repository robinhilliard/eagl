...
defmodule GLBLoaderExample do
  ...
  # Ensure inets application is started for HTTP requests
  def ensure_inets_started do
    case Application.ensure_all_started(:inets) do
      {:ok, _} -> :ok
      {:error, reason} ->
        IO.puts("Warning: Could not start :inets application: #{inspect(reason)}")
        :error
    end
  end
  ...
  def run do
    ...
    case GLBLoader.parse(@chair_url, timeout: 30_000) do
      ...
      {:error, reason} ->
        ...
        # If it's an httpc-related error, suggest alternatives
        if String.contains?(to_string(reason), "http_util") do
          IO.puts("\n💡 HTTP client compatibility issue detected.")
          IO.puts("The default :httpc client may not be compatible with this Erlang/OTP version.")
          IO.puts("You can try installing an alternative HTTP client:")
          IO.puts("  mix deps.get req  # Then restart this example")
          IO.puts("Or use a local GLB file instead of downloading from the web.")
        end
    end
  end
  ...
  def demo_auto_detection do
    ...
    case GLBLoader.parse(@chair_url, timeout: 30_000) do
      ...
      {:error, reason} ->
        ...
        if String.contains?(to_string(reason), "http_util") do
          IO.puts("   💡 Consider installing 'req' or 'httpoison' for better HTTP client support")
        end
    end
